...
> **New here? Read [`SETUP.md`](SETUP.md)** — a friendly, step-by-step guide to standing this up in
> your own Fabric workspace from scratch (prereqs, Azure resources + roles, Key Vault, S3, run order,
> and the optional end-to-end test).
...
Notebooks are named `nb_<role>_<NN>_<name>` so the role is obvious and each group runs in order:
**setup** (one-time), **pipeline** (the recurring ingest loop), **ops** (on-demand tools + tests).

| Notebook | What it does | When it runs |
| --- | --- | --- |
...
| `nb_ops_04_search_examples` | How to query the index: keyword / filtered / **security-trimmed** / vector / hybrid / semantic | **Ops/Reference** — on-demand |
...
- `nb_setup_02_set_config.ipynb` is a **template**. Copy it to `notebooks/_local_set_config.ipynb`
  (gitignored via `notebooks/_local_*.ipynb`), fill real endpoints, and run once. The `config` table
  MERGE preserves operator overrides across `nb_setup_01` re-runs, so you only set them once.
...
**`nb_pipeline_02` ingests on the Spark *driver* using a bounded `ThreadPoolExecutor` (`max_concurrency`,
...
| --- | --- | --- |
| Document Intelligence | **Cognitive Services User** | `nb_pipeline_02` (extract) |
| Azure OpenAI | **Cognitive Services OpenAI User** | `nb_pipeline_02` (embeddings) |
| Azure AI Search | **Search Index Data Contributor** | `nb_setup_03`, `nb_pipeline_02`, `nb_pipeline_03` (read/write index + docs) |
